...
| 7. Freies Experimentieren & Ausblick | ~5 Min (Puffer) | Eigene Funktionen eingeben, Desmos-Tipp | Notebook Abschnitt 5 |
...
**Aufgabenüberblick:**
- **Teil A (Ableitungen):** A1 Grundableitungen (Potenz, Wurzel, $\frac{1}{x^2}$), A2 Kettenregel (Polynom, e-Funktion, **$\sin(2x)$**), A3 Produkt- und **Quotientenregel**, A4 ln-Ableitung, A5 höhere Ableitungen, A6 geometrische Bedeutung
- **Teil B (Kurvendiskussion):** B1 Nullstellen/Extrema/Wendepunkt von $x^3 - 3x$, B2 Skizze aus $f'$/$f''$-Bedingungen, B3 Graph von $f'$ lesen
- **Teil C (Integralrechnung):** C1 Stammfunktionen, C2 bestimmtes Integral

**Worauf achten:**
- Kann sie Potenzen umschreiben ($\sqrt{x} = x^{1/2}$, $\frac{1}{x^2} = x^{-2}$)?
...
1. Koordinatensystem mit $f(x) = \tfrac{1}{2}x^2$ erscheint.
2. Fester Punkt $P$ bei $x_0 = 1{,}5$ und beweglicher Punkt $Q$ bei $x_1 = 4$.
3. Sekante durch $P$ und $Q$ mit Steigungsdreieck ($\Delta x$, $\Delta y$) und berechneter Sekantensteigung.
4. Frage: *„Was passiert, wenn $x_1$ immer näher an $x_0$ rückt?"*
5. $Q$ wandert schrittweise zu $P$ → Sekante wird zur Tangente.
6. Formel: $f'(x_0) = \lim\limits_{x_1 \to x_0}$ (Differenzenquotient → Differentialquotient).
...
- Bezug zu A6 im Diagnosetest herstellen: Die Ableitung $f'(x_0)$ ist die Steigung der Tangente an $x_0$.
...
- Die Schülerin verschiebt den Slider für $x_0$ entlang der Kurve $f(x) = x^3 - 3x$.
...
- Dropdown-Menü mit verschiedenen Funktionen: $x^2 - 2x$, $x^3 - 3x$, $\sin(x)$, $e^x$, $\ln(x)$.
- Oberer Plot zeigt $f$, unterer zeigt $f'$ — grüne Punkte markieren Extrema/Nullstellen.

**Durchführung:**
- Mit $x^3 - 3x$ starten (bekannt aus Phase 4 und Diagnosetest B1).
- Dann $e^x$ zeigen: *„Was fällt dir auf?"* → Die Ableitung von $e^x$ ist wieder $e^x$!
...
- Slider für den Exponenten $n$ von $-2$ bis $5$ → zeigt $f(x) = x^n$ und $f'(x) = n \cdot x^{n-1}$.

**Übungsplan (an Diagnose-Ergebnis anpassen):**

| Regel | Eingabe zum Testen | Erwartung |
|-------|-------------------|-----------|
| Potenzregel | `x^4 - 2*x^2` | $4x^3 - 4x$ |
| Kettenregel | `(2*x + 3)^5` | $10(2x+3)^4$ |
| sin/cos (mit Kette) | `sin(2*x)` | $2\cos(2x)$ |
| Produktregel | `x^2 * exp(x)` | $(2x + x^2) \cdot e^x$ |
| Quotientenregel | `x / (x^2 + 1)` | $\frac{1 - x^2}{(x^2 + 1)^2}$ |
| e-Funktion | `exp(-2*x)` | $-2e^{-2x}$ |
| ln-Funktion | `ln(3*x)` | $\frac{1}{x}$ |
...
**Quotientenregel:** Im Diagnosetest (A3b) jetzt direkt geprüft. Bei Unsicherheit: Formel $\left(\frac{u}{v}\right)' = \frac{u'v - uv'}{v^2}$ an Tafel/Papier, dann `x / (x^2 + 1)` ins Eingabefeld eintippen und mit dem Handrechenweg vergleichen.

---

## Phase 7: Freies Experimentieren & Ausblick (~5 Min, Puffer)
