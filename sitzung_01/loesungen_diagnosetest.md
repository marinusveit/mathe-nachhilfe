...
*Prüft: Kann sie Wurzel als Potenz schreiben? Typischer Fehler: $f'(x) = \frac{1}{2x}$ oder gar nicht umgeschrieben.*

**c)** $f(x) = \dfrac{1}{x^2} = x^{-2}$
$$f'(x) = -2x^{-3} = -\frac{2}{x^3}$$
...
*Typischer Fehler: innere Ableitung ($\cdot\, 2$) vergessen.*
...
*Merksatz: $(u \cdot v)' = u'v + uv'$. Typischer Fehler: nur einzeln ableiten → $2x \cdot e^x$.*

**b)** $f(x) = \dfrac{x}{x^2 + 1}$

Mit $u = x$, $v = x^2 + 1$, $u' = 1$, $v' = 2x$:
$$f'(x) = \frac{u'v - uv'}{v^2} = \frac{1 \cdot (x^2+1) - x \cdot 2x}{(x^2+1)^2} = \frac{1 - x^2}{(x^2+1)^2}$$
*Merksatz: $\left(\frac{u}{v}\right)' = \frac{u'v - uv'}{v^2}$. Typische Fehler: Vorzeichen im Zähler verdreht ($uv' - u'v$), oder Nenner nicht quadriert.*
...
**Weg 1 (Kettenregel):** $f'(x) = \frac{1}{3x} \cdot 3 = \mathbf{\frac{1}{x}}$
**Weg 2 (Logarithmengesetz):** $\ln(3x) = \ln 3 + \ln x$ → $f'(x) = \mathbf{\frac{1}{x}}$
...
**$f'(x_0)$ ist die Steigung der Tangente an den Graphen von $f$ im Punkt $(x_0, f(x_0))$.**

Akzeptable Antworten:
- „Steigung der Tangente"
- „momentane Änderungsrate von $f$ bei $x_0$"
...
$f''(x) = 6x$
- $f''(-1) = -6 < 0$ → **Hochpunkt** $H(-1 \mid 2)$ (da $f(-1) = -1 + 3 = 2$)
- $f''(1) = 6 > 0$ → **Tiefpunkt** $T(1 \mid -2)$ (da $f(1) = 1 - 3 = -2$)

**c) Wendepunkt:**
$f''(x) = 6x = 0 \Rightarrow x = 0$, $f(0) = 0$ → **$WP(0 \mid 0)$**
($f'''(0) = 6 \neq 0$ bestätigt den Wendepunkt.)
...
→ **Hochpunkt** bei $(2 \mid 5)$. Graph: Parabel-artig nach unten geöffnet, Scheitel bei $(2, 5)$.
...
Gezeigt ist $f'(x) = 4x - x^2$ (Parabel, Nullstellen bei $0$ und $4$, Maximum bei $x = 2$):
- $f'(x) < 0$ für $x < 0$
...
**b)** $f(x) = e^{2x}$
$$F(x) = \tfrac{1}{2}\, e^{2x} \ (+\, C)$$
*Typischer Fehler: einfach $F(x) = e^{2x}$ schreiben (Faktor $\tfrac{1}{2}$ vergessen).*
