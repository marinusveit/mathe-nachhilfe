...
1. **Definitionsbereich** bestimmen (Nenner ≠ 0, Wurzel ≥ 0, ln > 0)
...
6. **Extremstellen**:
   ...
   - Hinreichend: f''(x₀) > 0 → Minimum, f''(x₀) < 0 → Maximum
   - Alternative: Vorzeichenwechsel von f' prüfen
7. **Wendepunkte**:
   - Notwendig: f''(x) = 0 lösen
   - Hinreichend: f'''(x₀) ≠ 0 ODER Vorzeichenwechsel von f''
...
10. **Randverhalten**: lim x→±∞, ggf. lim an Polstellen
...
- Bei f''(x₀) = 0: Kein Extremum ausschließen, sondern VZW von f' prüfen
- Definitionsbereich vergessen → falsche Nullstellen/Extrema
- Vorzeichen beim Randverhalten: e^(−x) → 0 für x→∞, nicht →∞
